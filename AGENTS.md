...
Project context for Codex sessions on this repo.
...
## Relationship to the parent `Websitegemms/AGENTS.md`

The parent folder's `AGENTS.md` describes a generic "calm/healing/welcoming" template (sage/clay palette, Fraunces/Cormorant type, full-bleed hero with embedded trust bar). **Those defaults do not apply here.** MVC has its own established brand identity:
